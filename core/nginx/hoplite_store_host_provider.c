...
enum {
    HOPLITE_STORE_HOST_NEW = 0,
    HOPLITE_STORE_HOST_DISABLED = 1,
    HOPLITE_STORE_HOST_READY = 2,
    HOPLITE_STORE_HOST_FAILED = 3,
    HOPLITE_STORE_HOST_CLOSED = 4
};

static hoplite_value_store_provider_t *hoplite_store_provider;
static int hoplite_store_state;

static int32_t
hoplite_store_host_invoke(const hoplite_host_call_v1_t *call)
{
    ...
    if (call == NULL
        ...
        || hoplite_store_state != HOPLITE_STORE_HOST_READY
        || hoplite_store_provider == NULL
...
static const uint8_t hoplite_store_service_name[] = "hoplite.store";

static const hoplite_host_provider_v1_t hoplite_store_host_provider = {
    HOPLITE_HOST_PROVIDER_ABI_VERSION,
    {
        hoplite_store_service_name,
        sizeof(hoplite_store_service_name) - 1
    },
    hoplite_store_host_invoke,
    NULL,
    0,
    NULL,
    NULL,
...
static int
hoplite_store_parse_limit(const char *name,
...
int32_t
hoplite_store_host_provider_register_sqlite_v1(
    const uint8_t *path,
    size_t path_len,
    ...
    size_t max_receipt_bytes)
{
    ...
    if (hoplite_store_state != HOPLITE_STORE_HOST_NEW) {
        return HOPLITE_STORE_HOST_PROVIDER_ERROR;
    }
    if (path == NULL || path_len == 0
        ...
        || hoplite_value_store_provider_abi_version()
            != HOPLITE_VALUE_STORE_PROVIDER_ABI_VERSION)
    {
        hoplite_store_state = HOPLITE_STORE_HOST_FAILED;
        return HOPLITE_STORE_HOST_PROVIDER_ERROR;
    }
    ...
    status = hoplite_host_provider_register_v1(&hoplite_store_host_provider);
    ...
    hoplite_store_provider = provider;
    hoplite_store_state = HOPLITE_STORE_HOST_READY;
    return HOPLITE_STORE_HOST_PROVIDER_OK;
}

int32_t
hoplite_store_host_provider_init_process_v1(void)
{
    const char *path;
    size_t max_value_bytes;
    size_t max_receipt_bytes;

    if (hoplite_store_state == HOPLITE_STORE_HOST_READY) {
        return HOPLITE_STORE_HOST_PROVIDER_OK;
    }
    if (hoplite_store_state == HOPLITE_STORE_HOST_DISABLED) {
        return HOPLITE_STORE_HOST_PROVIDER_DISABLED;
    }
    if (hoplite_store_state != HOPLITE_STORE_HOST_NEW) {
        return HOPLITE_STORE_HOST_PROVIDER_ERROR;
    }
    ...
    if (path == NULL || path[0] == '\0') {
        hoplite_store_state = HOPLITE_STORE_HOST_DISABLED;
        return HOPLITE_STORE_HOST_PROVIDER_DISABLED;
    }
    if (!hoplite_store_parse_limit(
            HOPLITE_STORE_MAX_VALUE_ENV,
            HOPLITE_STORE_DEFAULT_MAX_VALUE,
            &max_value_bytes)
        || !hoplite_store_parse_limit(
...
void
hoplite_store_host_provider_exit_process_v1(void)
{
    if (hoplite_store_provider != NULL) {
        hoplite_value_store_provider_close_v1(hoplite_store_provider);
        hoplite_store_provider = NULL;
    }
    if (hoplite_store_state == HOPLITE_STORE_HOST_READY) {
        hoplite_store_state = HOPLITE_STORE_HOST_CLOSED;
    }
}
