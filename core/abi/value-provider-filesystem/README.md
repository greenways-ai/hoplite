# Hoplite filesystem `hara.value` provider core
...
existing `hara.blob` provider. It does not create a second object copy, semantic
cache, Tahto store, or application-specific index.

## Verification path

```text
closed hara.value-request/1
...
```text
hara.value/object-missing
hara.value/maximum-exceeded
hara.value/digest-mismatch
hara.value/hta-invalid
hara.value/hta-noncanonical
hara.value/value-unsupported
hara.value/provider-failure
```
