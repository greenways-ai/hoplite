...
Select the exact `:hoplite/nchan` export in the serving profile. Hoplite verifies
...
The installed package is only a build input. `hoplite-server` starts from the
...
See [`hoplite.nchan`](/reference/hoplite-nchan/) for the packaged channel
constructor and [`hoplite.rtc`](/reference/hoplite-rtc/) for the
