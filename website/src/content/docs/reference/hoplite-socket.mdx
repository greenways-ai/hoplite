...
- worker-local keepalive pools with monotonic expiry, bounded capacity, and real
  reuse counts.

Bounded positive connect backlog, simultaneous one-reader/one-writer operation,
TLS, and UDP remain pending. See the
[OpenResty cosocket compatibility matrix](../openresty-cosocket-compatibility/)
...
Refusal, DNS failure, timeout, peer closure, and other network outcomes resolve
normally so mechanically translated OpenResty control flow remains natural.
...
| `:backlog` | May be omitted or zero. Positive bounded scheduling is pending in [#187](https://github.com/greenways-ai/hoplite/issues/187). |
...
DNS; only the remaining duration is available to the native connect stage.
...
`timeout`, `closed`, `host not found`, and `resolver not configured`.

The centralized platform-independent error catalogue, including refusal/reset,
same-direction busy, backlog, TLS, UDP, and policy outcomes, is tracked by
...
| `sock:connect(host, port, {pool=..., pool_size=...})` | `(await (socket/connect sock host port {:pool ... :pool-size ...}))` |
...
| Surface | Owner |
| --- | --- |
| Positive bounded connect backlog | [#187](https://github.com/greenways-ai/hoplite/issues/187) |
